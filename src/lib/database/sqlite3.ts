...
const SpreadsheetCheckConfig = sequelize.define(
  'SpreadsheetCheckConfig',
  {
    id: {
...
DiscordServer.hasMany(SpreadsheetCheckConfig, {
  foreignKey: 'serverId',
});
...
export class SQLiteProvider {
  async getDiscordServer(
    serverId: string,
  ): Promise<DiscordServerModel> {
    const dbObject = await DiscordServer.findOne({
      where: { serverId },
    });
    return {
  ...
  async insertDiscordServer(
    serverId: string,
    serverName: string,
  ): Promise<DiscordServerModel> {
    const dbObject = await DiscordServer.create({
      serverId,
      serverName,
    });
    return {
  ...
  async updateDiscordServer(
    ...
    const [affectedCount] = await DiscordServer.update(
      { serverName },
      { where: { serverId } },
    );
  ...
  async deleteDiscordServer(serverId: string): Promise<boolean> {
    const deletedCount = await DiscordServer.destroy({
      where: { serverId },
    });
  ...
  async getSpreadsheetCheckConfigs(serverId: string) {
    const dbObjects = await SpreadsheetCheckConfig.findAll({
      where: { serverId },
    });
  ...
  async insertSpreadsheetCheckConfig(
    configName: string,
    ...
    const dbObject = await SpreadsheetCheckConfig.create({
      configName,
      spreadsheetId,
      sheetName,
      fullSheetRange,
